...
// Tipe Data Mata Pelajaran (Sama seperti di halaman utama)
interface MapelItem {
  id: string;
  nama: string;
}

interface ModalHapusMapelProps {
  mapel: MapelItem | null;
  onClose: () => void;
}

export default function ModalHapusMapel({ mapel, onClose }: ModalHapusMapelProps) {
  // Jika tidak ada data yang dipilih, modal tidak akan muncul
  if (!mapel) return null;

  const handleConfirmDelete = () => {
    console.log(`Mata pelajaran "${mapel.nama}" berhasil dihapus.`);
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl w-full max-w-md shadow-2xl p-6 md:p-8 animate-fade-in z-10 text-center">
        ...
        {/* Teks Peringatan */}
        <h3 className="text-xl font-extrabold text-black mb-2">Hapus Mata Pelajaran?</h3>
        <p className="text-gray-500 text-sm mb-6 leading-relaxed">
          Apakah Anda yakin ingin menghapus mata pelajaran <span className="font-bold text-gray-800">"{mapel.nama}"</span>? 
          <br />
