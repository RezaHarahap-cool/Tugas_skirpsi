...
// Tipe Data Kepsek / Staf (Sesuaikan dengan yang ada di halaman utama)
...
export default function ModalHapusKepsek({ kepsek, onClose }: ModalHapusKepsekProps) {
  // Jika tidak ada data yang dipilih, modal tidak akan muncul
  if (!kepsek) return null;

  const handleConfirmDelete = () => {
    console.log(`Data pimpinan/staf bernama ${kepsek.nama} berhasil dihapus.`);
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl w-full max-w-md shadow-2xl p-6 md:p-8 animate-fade-in z-10 text-center">
        ...
        {/* Teks Peringatan */}
        <h3 className="text-xl font-extrabold text-black mb-2">Hapus Data Pimpinan?</h3>
        <p className="text-gray-500 text-sm mb-6 leading-relaxed">
          Apakah Anda yakin ingin menghapus data <span className="font-bold text-gray-800">"{kepsek.nama}"</span> yang menjabat sebagai <span className="font-semibold text-gray-700">{kepsek.tugas}</span>? 
          <br />
