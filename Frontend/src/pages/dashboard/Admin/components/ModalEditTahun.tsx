...
interface ModalEditTahunProps {
  tahunAjaran: TahunAjaranItem | null;
  onClose: () => void;
}

export default function ModalEditTahun({ tahunAjaran, onClose }: ModalEditTahunProps) {
  // State lokal untuk menampung data yang sedang diedit
  const [formData, setFormData] = useState<TahunAjaranItem | null>(null);

  // Otomatis isi form dengan data tahun ajaran yang diklik
  useEffect(() => {
    if (tahunAjaran) {
      setFormData(tahunAjaran);
    }
  }, [tahunAjaran]);

  // Jika tidak ada data yang dipilih, jangan tampilkan apa-apa
  if (!tahunAjaran || !formData) return null;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    console.log("Data Tahun Ajaran yang di-update:", formData);
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      {/* Kotak Form Modal */}
      <div className="relative bg-white rounded-2xl w-full max-w-md shadow-2xl p-6 md:p-8 animate-fade-in z-10 custom-scrollbar">
        
        {/* Header Modal */}
        <div className="flex justify-between items-center mb-6">
          <h3 className="text-xl font-extrabold text-black">Edit Tahun Ajaran</h3>
          <button 
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            >
              <option value="Aktif" className="text-green-600">Aktif</option>
              <option value="Nonaktif" className="text-red-600">Nonaktif</option>
            </select>
          </div>
